...
export default function RescueAdoptionCommentsSection({
  postId,
  postType,
  comments = [],
  total = 0,
  ...
  const handleAddComment = async () => {
    if (!user) { toast(t("comments.login"), "error"); return; }
    if (!newComment.trim()) { toast(t("comments.empty"), "error"); return; }

    setSubmitting(true);
    try {
      await rescueAdoptionAPI.addComment(postId, postType, newComment.trim());
  ...
  const handleReport = async (commentId: any) => {
    ...
    try {
      await rescueAdoptionAPI.reportComment(commentId, reportReason);
    ...
    <div className="border-t border-[var(--border)] pt-6">
          ...
          <div className="flex gap-3 mb-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center justify-between mt-2">
                <span className="text-xs text-[var(--text-secondary)]">{newComment.length}/500</span>
                <button
      ...
      {comments.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-[var(--text-secondary)]">{t("comments.empty")}</p>
        </div>
      ) : (
            ...
            <div key={comment.id} className="p-4 bg-[var(--bg-secondary)] rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div
                  ...
                  style={comment.profile_picture ? { display: "none" } : { display: "flex" }}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="font-semibold text-[var(--text-primary)] truncate">{comment.name || t("comments.anonymous")}</span>
                      <span className="text-xs text-[var(--text-secondary)] flex-shrink-0">{formatTimeAgo(comment.created_at)}</span>
                    </div>
                    {user && user.id !== comment.user_id && (
                      <button
                        ...
                        className="text-xs text-[var(--text-secondary)] hover:text-[var(--danger)] transition-colors flex-shrink-0"
                      >
                        {reportingId === comment.id ? t("comments.cancel") : t("comments.report")}
                      </button>
                    )}
                  </div>
                  <p className="text-[var(--text-primary)] text-sm break-words">{comment.comment_text}</p>
