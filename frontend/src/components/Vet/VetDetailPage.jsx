...
export default function VetDetailPage({ vetId, open, onClose, onAuthRequired }) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ position: 'relative', background: '#0a0d12' }}>
              {hasCover
                ? <img src={coverUrl} alt={`${vet.name} cover`} style={{ width: '100%', display: 'block', objectFit: 'cover', maxHeight: 280 }} onError={e => e.target.style.display = 'none'} />
                    ...
                    {clinicVets.map((cv) => {
                      ...
                      return (
                        <div key={cv.id} style={{ padding: 16, background: 'var(--bg-elevated)', borderRadius: 12, border: '1px solid var(--border)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                          {cvAvatar
                            ? <img src={cvAvatar} alt={cv.name} style={{ width: 48, height: 48, borderRadius: 10, objectFit: 'cover', flexShrink: 0 }} onError={e => e.target.style.display = 'none'} />
