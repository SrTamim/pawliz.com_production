...
function ImageUploadBox({ src, label, uploadLabel, onUpload, loading }) {
  const ref = useRef();
  return (
    <div
...
export default function VetProfileDetails({ vet, qualifications, documents, clinicVets, clinicContacts, owner, onRefresh }) {
  ...
  const [newClinicVet, setNewClinicVet] = useState({ name: "", designation: "", bvc_reg_number: "", bmdc_reg_number: "", checkup_start: "", checkup_end: "", weekly_holidays: [] });
  const [newClinicVetImage, setNewClinicVetImage] = useState(null);
  ...
  const toggleHoliday = (day) => {
    setProfile((p) => ({
      ...
      weekly_holidays: p.weekly_holidays.includes(day)
        ? p.weekly_holidays.filter((d) => d !== day)
  ...
  const saveProfile = async () => {
    setSaving(true);
    try {
      const servicesArr = profile.services.split(",").map((s) => s.trim()).filter(Boolean);
  ...
  const uploadCover = async (file) => {
    setImgLoading((l) => ({ ...l, cover: true }));
    try {
      await vetDashboardAPI.uploadCoverImage(file);
      toast("Cover image updated");
      onRefresh();
    } catch (e) { toast(e.message, "error"); }
    finally { setImgLoading((l) => ({ ...l, cover: false })); }
  };

  const uploadVetImg = async (file) => {
    setImgLoading((l) => ({ ...l, vet: true }));
    try {
      await vetDashboardAPI.uploadVetImage(file);
      toast("Image updated");
      onRefresh();
    } catch (e) { toast(e.message, "error"); }
    finally { setImgLoading((l) => ({ ...l, vet: false })); }
  };
  ...
  const saveClinicVet = async () => {
    ...
    try {
      ...
      setNewClinicVet({ name: "", designation: "", bvc_reg_number: "", bmdc_reg_number: "", checkup_start: "", checkup_end: "", weekly_holidays: [] });
  ...
  const removeClinicVet = async (id) => {
    ...
    <div>
      ...
      <Section title={t("profileForm.clinicInfoSection") || "Basic Information of Clinic"}>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
          <Field label={t("profileForm.clinicName")}>
            <input style={inputStyle} value={profile.name} onChange={(e) => setP("name", e.target.value)} placeholder="Clinic name" />
          </Field>
          <Field label={t("profileForm.clinicRegNum")}>
            <input style={inputStyle} value={profile.clinic_reg_number} onChange={(e) => setP("clinic_reg_number", e.target.value)} placeholder="Hospital / Clinic registration number" />
          </Field>
        </div>
        ...
        <Field label={t("profileForm.services")}>
          <input style={inputStyle} value={profile.services} onChange={(e) => setP("services", e.target.value)} placeholder="e.g. Surgery, Vaccination, Dental" />
        </Field>
        <Field label={t("profileForm.address")}>
          <input style={inputStyle} value={profile.address} onChange={(e) => setP("address", e.target.value)} placeholder="Clinic address" />
        </Field>
      ...
      <Section title={t("profileForm.clinicHours")}>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 16 }}>
          <Field label={t("profileForm.startTime")}>
            <select style={inputStyle} value={profile.checkup_start} onChange={(e) => setP("checkup_start", e.target.value)}>
              {TIME_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </Field>
          <Field label={t("profileForm.endTime")}>
            <select style={inputStyle} value={profile.checkup_end} onChange={(e) => setP("checkup_end", e.target.value)}>
              {TIME_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </Field>
        </div>
      ...
      <Section title={t("profileForm.clinicContactsSection")}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "160px 1fr auto", gap: 10, marginTop: 12 }}>
          <select value={newContact.contact_type} onChange={(e) => setNewContact((n) => ({ ...n, contact_type: e.target.value }))} style={inputStyle}>
            {CONTACT_TYPES.map((ct) => <option key={ct} value={ct}>{ct.charAt(0).toUpperCase() + ct.slice(1)}</option>)}
          </select>
          <input style={inputStyle} value={newContact.contact_value} onChange={(e) => setNewContact((n) => ({ ...n, contact_value: e.target.value }))} placeholder="Phone / Email / URL" />
          ...
          <div style={{ padding: "18px", background: "var(--bg-page)", borderRadius: 10, border: "1px solid var(--border)", marginTop: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div>
                ...
                <input style={inputStyle} value={newClinicVet.name} onChange={(e) => setNewClinicVet((n) => ({ ...n, name: e.target.value }))} placeholder="Dr. Name" />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>{t("profileForm.vetDesignationField")} *</label>
                <input style={inputStyle} value={newClinicVet.designation} onChange={(e) => setNewClinicVet((n) => ({ ...n, designation: e.target.value }))} placeholder="Veterinary Surgeon" />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>BVC Registration Number</label>
                <input style={inputStyle} value={newClinicVet.bvc_reg_number} onChange={(e) => setNewClinicVet((n) => ({ ...n, bvc_reg_number: e.target.value }))} placeholder="BVC reg. number" />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>BMDC Registration Number</label>
                <input style={inputStyle} value={newClinicVet.bmdc_reg_number} onChange={(e) => setNewClinicVet((n) => ({ ...n, bmdc_reg_number: e.target.value }))} placeholder="BMDC reg. number" />
              </div>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <label style={{ cursor: "pointer", fontSize: 13, color: "var(--accent)", padding: "6px 12px", borderRadius: 6, border: "1px solid var(--accent)", background: "transparent" }}>
                    ...
                    <input type="file" accept="image/*" onChange={(e) => setNewClinicVetImage(e.target.files[0])} style={{ display: "none" }} />
                  </label>
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>{t("profileForm.checkupStart")}</label>
                <select style={inputStyle} value={newClinicVet.checkup_start} onChange={(e) => setNewClinicVet((n) => ({ ...n, checkup_start: e.target.value }))}>
                  {TIME_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--text-secondary)", display: "block", marginBottom: 5 }}>{t("profileForm.checkupEnd")}</label>
                <select style={inputStyle} value={newClinicVet.checkup_end} onChange={(e) => setNewClinicVet((n) => ({ ...n, checkup_end: e.target.value }))}>
                  {TIME_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
            </div>
                ...
                {DAYS.map((day) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setNewClinicVet((n) => ({
                        ...n,
                        weekly_holidays: h.includes(day) ? h.filter((d) => d !== day) : [...h, day],
                      }));
                    }}
                    style={{
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {newClinicVetQuals.map((q, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr auto", gap: 8, marginBottom: 6 }}>
                  <input style={inputStyle} value={q.qualification} onChange={(e) => setNewClinicVetQuals((arr) => arr.map((x, j) => j === i ? { ...x, qualification: e.target.value } : x))} placeholder={t("profileForm.degreeCert")} />
                  <input style={inputStyle} value={q.institute} onChange={(e) => setNewClinicVetQuals((arr) => arr.map((x, j) => j === i ? { ...x, institute: e.target.value } : x))} placeholder={t("profileForm.institute")} />
                  <button style={btnDanger} onClick={() => setNewClinicVetQuals((arr) => arr.filter((_, j) => j !== i))}>✕</button>
                </div>
              ))}
              <button style={btnSecondary} onClick={() => setNewClinicVetQuals((arr) => [...arr, { qualification: "", institute: "" }])}>
      ...
      <Section title={t("profileForm.socialMediaSection")}>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
          <Field label={t("profileForm.facebookUrl")}>
            <input style={inputStyle} value={profile.social_facebook} onChange={(e) => setP("social_facebook", e.target.value)} placeholder="https://facebook.com/yourpage" />
          </Field>
          <Field label={t("profileForm.instagramUrl")}>
            <input style={inputStyle} value={profile.social_instagram} onChange={(e) => setP("social_instagram", e.target.value)} placeholder="https://instagram.com/yourhandle" />
          </Field>
          <Field label={t("profileForm.linkedinUrl")}>
            <input style={inputStyle} value={profile.social_linkedin} onChange={(e) => setP("social_linkedin", e.target.value)} placeholder="https://linkedin.com/in/yourprofile" />
          </Field>
          <Field label={t("profileForm.whatsappNum")}>
            <input style={inputStyle} value={profile.social_whatsapp} onChange={(e) => setP("social_whatsapp", e.target.value)} placeholder="+880 1XXXXXXXXX" />
          </Field>
          <Field label={t("profileForm.website") || "Website"}>
            <input style={inputStyle} value={profile.website} onChange={(e) => setP("website", e.target.value)} placeholder="https://yourclinic.com" />
          </Field>
        </div>
      </Section>
      ...
      <Section title={t("profileForm.certificatesSection")}>
          ...
          <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "var(--bg-elevated)", borderRadius: 8, marginBottom: 8 }}>
            <div>
              <div style={{ fontWeight: 500, fontSize: 13, color: "var(--text-primary)" }}>
                {DOC_TYPES_CLINIC.find((t) => t.value === d.doc_type)?.label || d.doc_type}
              </div>
              <a
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 12, alignItems: "center" }}>
          ...
          >
            {DOC_TYPES_CLINIC.map((t) => (
              <option key={t.value} value={t.value}>{t.label}</option>
            ))}
          </select>
          <button style={btnPrimary} onClick={() => docRef.current.click()}>{t("profileForm.upload")}</button>
          <input ref={docRef} type="file" style={{ display: "none" }} onChange={(e) => e.target.files[0] && uploadDoc(e.target.files[0])} />
        </div>
      </Section>
